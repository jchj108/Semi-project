...
$rDisplay = !isset($_GET['rDisplay'])? 10 : $_GET['rDisplay'];
...
$rKey= "f6680abbb16ce317f4ef787b7e83e952";
...
<input type="hidden" name="rDisplay" value="<?=$rDisplay?>"/><br />
<input type="hidden" name="rStart" value="1"/><br />
...
<a href="?<?echo "rQuery=".$rQuery."&amp;rTarget=".$rTarget."&amp;rStart=".($rStart-$rDisplay)."&amp;rDisplay=".$rDisplay;?>">pre</a>
<a href="?<?echo "rQuery=".$rQuery."&amp;rTarget=".$rTarget."&amp;rStart=".($rStart+$rDisplay)."&amp;rDisplay=".$rDisplay;?>">next</a>
<?
...
// 츌력
	...
	$pquery = "&key=".$rKey."&query=".$rQuery."&target=".$rTarget."&start=".$rStart."&display=".$rDisplay;

	$channel = $xml->xmlOpen($pquery,'channel');
	echo $channel['lastBuildDate'][0]['value'].'<br />총 ';
	echo $channel['total'][0]['value'].'개의 검색 결과<br />';
	if($channel['total'][0]['value']>0){
		echo (int)($channel['start'][0]['value']/$channel['display'][0]['value']+1).'번째 페이지<br />';
		echo $channel['display'][0]['value'].'<br />';
		...
		for($x=0; $x<$count; $x++) {
			echo "<div style=\"float:left;width:200px;height:200px;\"><a href=\"".str_replace('&','&amp;',$item['link'][$x]['value'])."\"><img src=\"".$item['thumbnail'][$x]['value']."\" alt=\"".$item['title'][$x]['value']."\" title=\"".$item['title'][$x]['value']."\" /><br />";
//			if(mb_strcut($item['title'][$x]['value'], 0, 30, 'utf-8')) echo '...';
			echo "<span style=\"font-size:10pt;\">".$item['sizewidth'][$x]['value']."×".$item['sizeheight'][$x]['value']."<br />".strcut_utf8($item['title'][$x]['value'],20)."</span></a><br />";
			echo '</div>';
		}
	}
